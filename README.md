...
A modern storefront for **Envishon**, a US-based company that dropships
...
The site uses the official Envishon logo files, stored in
...
| `wordmark.png` | Metallic "ENVISHON" wordmark |
...
"ENVISHON" wordmark in Space Grotesk. To use the metallic wordmark image
...
Follows the **Envishon Brand Guide v1.0**. All tokens live in
...
- **Colors:** Titanium Silver `#C5C8CC` (primary — headings/logo) on a Near
  ...
  `#8A8F98` body. **Orange is the spark** — Molten Orange `#7C3AED` for
  buttons/links/key actions, Signal `#8B5CF6` for hover, Ember `#C4B5FD` for
...
src/
  app/
    ...
    about/page.tsx             # About ("Envishon = layers")
